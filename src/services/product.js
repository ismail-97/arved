...
const getProducts = async () => {
    const response = await axios.get(`${baseUrl}`, {headers: { Authorization: getToken()}})
    return response.data
}
const getProductsReport = async (products) => {
    const response =
        axios.post(`/admin/filter/create-pdf`, products,
            {
            ...
            .then(response => {
                saveAs(new Blob([response.data], { type: "application/pdf" }), "report.pdf")
            });
    return response.data
}
const getProductDocument = async (fileId) => {
    const response =
        axios.get(`${baseUrl}/files/${fileId}`,
            {
                responseType: 'arraybuffer',
                headers: {
                    Authorization: getToken(),
            ...
            .then(response => {
                ...
                saveAs(new Blob([response.data], { type: "application/pdf" }), `${response.headers['content-disposition']}`)
            });
...
const editProduct = async (editedProduct, productId) => {
    ...
    const response =
        await axios.put(`${baseUrl}/${productId}`,
            formData,
            {
                headers: {
                    Authorization: getToken(),
...
const deleteAProduct = async (productId) => {
    const response = await axios.delete(`${baseUrl}/${productId}`, {headers: { Authorization: getToken() }})
    return response.data
}

const searchProducts = async (criteria) => {
    const response = await axios.post(`http://localhost:3001/admin/filter`, criteria, { headers: {Authorization: getToken()}})
    return response.data
}
